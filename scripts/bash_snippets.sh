...
# https using self generated cert
openssl req -new -x509 -keyout server.pem -out server.pem -days 365 -nodes
python -c 'import BaseHTTPServer as BS, SimpleHTTPServer as SS, ssl;httpd=BS.HTTPServer(("10.9.8.1", 443), SS.SimpleHTTPRequestHandler);httpd.socket=ssl.wrap_socket(httpd.socket, certfile='./server.pem', server_side=True);httpd.serve_forever()'

# webdav http in place of smb
wsgidav --auth=anonymous --host=10.9.8.1 --port=443 --root=./
